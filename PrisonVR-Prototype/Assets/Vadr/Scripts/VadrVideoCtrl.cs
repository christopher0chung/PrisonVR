using UnityEngine;
using System.Runtime.InteropServices;
using System;
using System.Collections;
using System.Threading;
using System.Collections.Generic;
using System.Net;
using System.IO;
using UnityEngine.SceneManagement;
using VadrSdk;

public class VadrVideoCtrl : MonoBehaviour {
	
	private string m_strFileName;
	private GameObject m_TargetMaterial = null; 
	private Texture2D m_VideoTexture = null;
	private Texture2D m_VideoTextureDummy = null;
	public MEDIAPLAYER_STATE m_CurrentState { get; set; }
	private int m_iCurrentSeekPosition;
	private float m_fVolume = 1.0f;
	
	private bool m_bFullScreen = false;//Please use only in FullScreen prefab.
	private bool m_bSupportRockchip = true; //Using a device support Rochchip or Low-end devices
	//(Reason 1 : Not directly play in StreamingAssets)
	//(Reason 2 : Video buffer is RGB565 only supported)
	
	public delegate void VideoEnd();
    public delegate void InterstitialEnd();
    public static event InterstitialEnd VadrInterstitialEnd;
    public delegate void VideoReady();
	public delegate void VideoError(MEDIAPLAYER_ERROR errorCode, MEDIAPLAYER_ERROR errorCodeExtra);
	public delegate void VideoFirstFrameReady();
    public AndroidJavaObject javaObjAds;
    public VideoReady OnReady;
	public static VideoEnd OnEnd;
	public VideoError OnVideoError;
	public VideoFirstFrameReady OnVideoFirstFrameReady;
    private VastParser vast;
    private float previousVolume;
    private GameObject progressBarVideo;
    private GameObject progressBarInternalVideo;
    private GameObject progressBarExternalVideo;
    private Renderer progressBarRendererExternalVideo;
    private Renderer progressBarRendererInternalVideo;
    private byte[] companionImg;
    CentralAdManager adManager;
    bool isRedirected;
    public delegate void VadrAppStoreEventHandler(string url);
    public static event VadrAppStoreEventHandler VadrAppStore;
	public delegate void VadrVideoiOSWebviewEventHandler(string url);
	public static event VadrVideoiOSWebviewEventHandler VadrVideoiOSWebview;
    public delegate void InterstitialEndGame();
    public static event InterstitialEndGame VadrInterstitial;

	private bool iPhoneTextureInverted = false;
#if UNITY_IPHONE || UNITY_TVOS

	private int m_iPauseFrame;
#endif


#if UNITY_ANDROID && !UNITY_EDITOR && UNITY_5
	[DllImport ("BlueDoveMediaRender")]
	private static extern void InitNDK();

#if UNITY_5_2
	[DllImport ("BlueDoveMediaRender")]
	private static extern IntPtr EasyMovieTextureRender();
#endif
#endif



    private int m_iAndroidMgrID;
	private bool m_bIsFirstFrameReady;

    public enum MEDIAPLAYER_ERROR
    {
		MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK = 200,
		MEDIA_ERROR_IO           = -1004,
        MEDIA_ERROR_MALFORMED    = -1007,
        MEDIA_ERROR_TIMED_OUT    = -110,
        MEDIA_ERROR_UNSUPPORTED  = -1010,
        MEDIA_ERROR_SERVER_DIED  = 100,
        MEDIA_ERROR_UNKNOWN      = 1
    }
	
	public enum MEDIAPLAYER_STATE
    {
		NOT_READY       = 0,
		READY           = 1,
        END     		= 2,
        PLAYING         = 3,
        PAUSED          = 4,
        STOPPED         = 5,
        ERROR           = 6,
        ERROR_STREAM    = 7,
    }
	
	public enum MEDIA_SCALE
	{
		SCALE_X_TO_Y	= 0,
		SCALE_X_TO_Z	= 1,
		SCALE_Y_TO_X	= 2,
		SCALE_Y_TO_Z	= 3,
		SCALE_Z_TO_X	= 4,
		SCALE_Z_TO_Y	= 5,
		SCALE_X_TO_Y_2	= 6,
	}
	
	bool m_bFirst = false;
	
	private MEDIA_SCALE m_ScaleValue;
	private GameObject m_objResize = null;
	private bool m_bLoop = false;
	private bool m_bAutoPlay = true;
	private bool m_bStop = false;



    	void Awake(){

		if( SystemInfo.deviceModel.Contains("rockchip"))
		{
			m_bSupportRockchip = true;
		}
		else
		{
			m_bSupportRockchip = false;
		}


//		#if UNITY_IPHONE || UNITY_TVOS
//		if(m_TargetMaterial!=null)
//		{
//			Debug.Log("Command to invert texture ran");
//			if(!iPhoneTextureInverted && m_TargetMaterial.GetComponent<MeshFilter>() != null)
//			{
//				iPhoneTextureInverted = true;
//				Debug.Log("Video debug: Inverting texture");
//				Vector2 [] vec2UVs= m_TargetMaterial.GetComponent<MeshFilter>().mesh.uv;
//				
//				for(int i = 0; i < vec2UVs.Length; i++)
//				{
//					vec2UVs[i] = new Vector2(vec2UVs[i].x, 1.0f -vec2UVs[i].y);
//				}
//				
//				m_TargetMaterial.GetComponent<MeshFilter>().mesh.uv = vec2UVs;
//			}
//			
////			if(m_TargetMaterial.GetComponent<RawImage>() != null)
////			{
////				m_TargetMaterial.GetComponent<RawImage>().uvRect = new Rect(0,1,1,-1);
////			}
//		}
//		#endif



		
		
	}
    // Use this for initialization
    void Start()
    {
        init();
    }


    public void init() {
        m_VideoTexture = null;
        m_VideoTextureDummy = null;
        m_objResize = null;
        m_bLoop = false;
        m_bAutoPlay = true;
        m_bStop = false;
        m_TargetMaterial = gameObject;
        Call_SetUnityActivity();
        isRedirected = false;
        for (int i = 0; i < transform.childCount; i++)
        {
            Transform child = transform.GetChild(i);
            if (child.name == "ProgressBarVideo")
            {
                progressBarVideo = child.gameObject;
                for (int j = 0; j < child.childCount; j++)
                {
                    Transform grandChild = child.GetChild(j);
                    if (grandChild.name == "ProgressBarSpriteInternalVideo")
                    {
                        progressBarInternalVideo = grandChild.gameObject;
                        progressBarRendererInternalVideo = progressBarInternalVideo.GetComponent<Renderer>();
                    }
                    if (grandChild.name == "ProgressBarSpriteExternalVideo")
                    {
                        progressBarExternalVideo = grandChild.gameObject;
                        progressBarRendererExternalVideo = progressBarExternalVideo.GetComponent<Renderer>();
                    }
                }
            }
        }
        ShowStripes(false);
        vast = new VastParser();
        previousVolume = 0;
        SetVolume(0);
        previousVolume = 1;
        if (transform.name == "VideoAd")
        {
            if (InterstitialVideo.interstitialVast != null && InterstitialVideo.interstitialVast.inLineVideo.media.Count > 0)
            {
                string[] tempExt = InterstitialVideo.interstitialVast.inLineVideo.media[0].videoPath.Split('.');
                string extension = tempExt[tempExt.Length - 1];
                string path = GetPath() + "vadr" + Animator.StringToHash(InterstitialVideo.interstitialVast.inLineVideo.media[0].videoPath) + "." + extension;
                if (File.Exists(path))
                {
                    Load("file://" + path, InterstitialVideo.interstitialVast);
                    SetVolume(1);
                }
                InterstitialVideo.interstitialVast = null;
            }
        }


#if UNITY_ANDROID && !UNITY_EDITOR
		
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
			InitNDK();
#endif
		m_iAndroidMgrID = Call_InitNDK();
		Call_SetUnityActivity();
#endif


#if UNITY_ANDROID
        if (Application.dataPath.Contains(".obb"))
        {

            Call_SetSplitOBB(true, Application.dataPath);
        }
        else
        {
            Call_SetSplitOBB(false, null);
        }
#endif
    }
	public string GetPath()
	{
		string path = "";
		if (Application.platform == RuntimePlatform.Android) {
			AndroidJavaClass unity = new AndroidJavaClass ("com.unity3d.player.UnityPlayer");
			AndroidJavaObject currentActivity = unity.GetStatic<AndroidJavaObject> ("currentActivity");
			AndroidJavaClass jc = new AndroidJavaClass ("com.vadrnet.unitysdk.MainActivity");
			path = jc.CallStatic<string> ("getFilePathVideo");

		} else if (Application.platform == RuntimePlatform.IPhonePlayer) {
			path = VadrSDKLink._getChacheDirectoryString () + "/VadrAdCache/";
		}
		else
		{
			path = "VadrAdCache/";
		}
		if (!Directory.Exists (path)) {
			Directory.CreateDirectory (path);
		}
		return path;

	}
    void OnApplicationQuit()
	{
		if( System.IO.Directory.Exists( Application.persistentDataPath + "/Data" ) == true)
			System.IO.Directory.Delete( Application.persistentDataPath + "/Data", true);
	}
	
	bool m_bCheckFBO = false;
	
    void LateUpdate()
    {
        if (isRedirected && transform.name == "VideoAd")
        {
            StartCoroutine(LoadNewScene());
        }
    }

    public IEnumerator LoadNewScene()
    {
        if (transform.name == "VideoAd")
        {
            float timeNow = Time.realtimeSinceStartup;
            while (Time.realtimeSinceStartup - timeNow < 0.1f)
            {
                yield return 0;
            }
            if (VadrInterstitial != null)
            {
                VadrInterstitial();
            }
        }
    }

	void Update()
	{

        if ( string.IsNullOrEmpty(m_strFileName) )
		{
			return;
		}

//        if (m_bFirst == false)
//		{
//		
//			
//			
//			
//			
//			string strName = m_strFileName.Trim();
//#if UNITY_IPHONE || UNITY_TVOS
//			if (strName.StartsWith("http",StringComparison.OrdinalIgnoreCase))
//			{
//				StartCoroutine( DownloadStreamingVideoAndLoad(strName) );
//			}
//			else
//				Call_Load(strName,0);
//			
//#endif
//#if UNITY_ANDROID
//			
//			if(m_bSupportRockchip)
//			{
//				Call_SetRockchip(m_bSupportRockchip);
//				
//				if(strName.Contains("://"))
//				{
//					Call_Load(strName,0);
//				}
//				else
//				{
//					//Call_Load(strName,0);
//					StartCoroutine( CopyStreamingAssetVideoAndLoad(strName));
//				}
//			
//			}
//			else
//			{
//				Call_Load(strName,0);
//			}
//		
//	 		
//#endif
//			Call_SetLooping(m_bLoop);
//			m_bFirst = true;
//			
//		
//		}
	
		
		if(m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
		{
            if (m_bCheckFBO == false)
			{
				if(Call_GetVideoWidth() <= 0 || Call_GetVideoHeight() <= 0)
				{
					return;
				}
				
				Resize();
			
				if(m_VideoTexture != null)
				{
					
					//Destroy(m_VideoTexture);

					if( m_VideoTextureDummy != null)
					{
						Destroy(m_VideoTextureDummy);
						m_VideoTextureDummy = null;
					}

					m_VideoTextureDummy = m_VideoTexture;
					m_VideoTexture = null;
					
					
				}

#if UNITY_ANDROID
				if(m_bSupportRockchip)
				{
					m_VideoTexture = new Texture2D(Call_GetVideoWidth(),Call_GetVideoHeight(),TextureFormat.RGB565,false);
				}
				else
				{
					m_VideoTexture = new Texture2D(Call_GetVideoWidth(),Call_GetVideoHeight(),TextureFormat.RGBA32,false);
				}
				
				m_VideoTexture.filterMode = FilterMode.Bilinear;
			    m_VideoTexture.wrapMode = TextureWrapMode.Clamp;
				
#if UNITY_5_2
				Call_SetUnityTexture((int)m_VideoTexture.GetNativeTexturePtr());
#else
				Call_SetUnityTexture(m_VideoTexture.GetNativeTextureID());
#endif
				#endif
				Call_SetWindowSize();
				m_bCheckFBO = true;
				
			}
			

			Call_UpdateVideoTexture();
			
			
	
			m_iCurrentSeekPosition = Call_GetSeekPosition();
			
			
			// if( m_strFileName.StartsWith("rtsp",StringComparison.OrdinalIgnoreCase) || m_strFileName.StartsWith("http",StringComparison.OrdinalIgnoreCase))
			// {
			// }
			// else
			// {
			// 	if(m_iCurrentSeekPosition > (Call_GetDuration() - 500) & m_bLoop == true & (m_CurrentState == MEDIAPLAYER_STATE.PLAYING))
			// 	{
			// 		SeekTo(0);
			// 	}
	
			// }
			
		}
	
		

		if(m_CurrentState != Call_GetStatus() )
		{
			
			m_CurrentState = Call_GetStatus();
			
			

			if(m_CurrentState == MEDIAPLAYER_STATE.READY)
			{

				if(OnReady != null)
					OnReady();

                if (m_bAutoPlay)
                {
                    if (vast.impressionUrl.Count > 0)
                    {
                        makeRequest(vast.impressionUrl);
                    }
                    if (vast.inLineVideo.creativeViewUrl.Count > 0)
                    {
                        makeRequest(vast.inLineVideo.creativeViewUrl);
                    }
                    if (vast.inLineVideo.startUrl.Count > 0)
                    {
                        makeRequest(vast.inLineVideo.startUrl);
                    }
                    Call_Play(0);
                    if (transform.name != "VideoAd")
                    {
                        SetVolume(0);
                    }
                    //Pause();
                    StartCoroutine(CheckProgress());
                }



			}
			else if(m_CurrentState == MEDIAPLAYER_STATE.END)
			{
                if (vast.inLineVideo.completeUrl.Count > 0)
                {
                    makeRequest(vast.inLineVideo.completeUrl);
                }
                if(companionImg != null)
                {
					changeTextureAfterInverting(companionImg);
                    if(vast.inLineVideo.companion[0].createViewUrl.Count > 0)
                    {
                        makeRequest(vast.inLineVideo.companion[0].createViewUrl);
                    }
                }
                Debug.Log("Name: "+ transform.name);
                if (transform.name == "VideoAd" && !isRedirected)
                {
                    Debug.Log("Video Ended");
                    if(VadrInterstitialEnd != null)
                        VadrInterstitialEnd();
                    StartCoroutine(Redirect());
                }
                if (isRedirected && transform.name == "VideoAd")
                {
                    StartCoroutine(LoadNewScene());
                }
                if (OnEnd != null)
					OnEnd();
				
				if(m_bLoop == true)
				{
					Call_Play(0);
				}
            }
			else if(m_CurrentState == MEDIAPLAYER_STATE.ERROR)
			{
                if (vast.errorUrl.Count > 0)
                {
                    for (int i = 0; i < vast.errorUrl.Count; i++)
                    {
                        if (vast.errorUrl[i].Contains("[ERRORCODE]"))
                        {
                            string url = vast.errorUrl[i].Replace("[ERRORCODE]", "400"); //This is general Video Player error
                            makeRequest(url);
                        }
                    }
                }
                StartCoroutine(LoadNewScene());
                OnError( (MEDIAPLAYER_ERROR)Call_GetError() ,(MEDIAPLAYER_ERROR)Call_GetErrorExtra() );
			}
			
		}
	}
	
    IEnumerator Redirect()
    {
        float timeNow = Time.realtimeSinceStartup;
        while (Time.realtimeSinceStartup - timeNow < 1.5f)
        {
            yield return 0;
        }
        Click();
    }

	public void Resize()
	{
		if(m_CurrentState != MEDIAPLAYER_STATE.PLAYING)
			return;

		if (Call_GetVideoWidth() <= 0 || Call_GetVideoHeight() <= 0)
        {
            return;
        }
		
		if(m_objResize != null)
		{
			int iScreenWidth = Screen.width;
			int iScreenHeight = Screen.height;
			
			float fRatioScreen = (float)iScreenHeight / (float)iScreenWidth;
			int iWidth = Call_GetVideoWidth();
			int iHeight = Call_GetVideoHeight();
			
			float fRatio = (float)iHeight / (float)iWidth;
			float fRatioResult = fRatioScreen / fRatio;
			
			
			if(m_bFullScreen)
			{
				if(fRatio < 1.0f)
				{
					if( fRatioScreen < 1.0f)
					{
						if( fRatio > fRatioScreen)
						{
							m_objResize.transform.localScale *= fRatioResult;
						}
					}
					
					m_ScaleValue = MEDIA_SCALE.SCALE_X_TO_Y;
				}
				else
				{
					if( fRatioScreen > 1.0f)
					{
						if( fRatio > fRatioScreen)
						{
							m_objResize.transform.localScale *= fRatioResult;
						}
					}
					
					m_ScaleValue = MEDIA_SCALE.SCALE_Y_TO_X;
				}
			}
			
			
			
			if( m_ScaleValue == MEDIA_SCALE.SCALE_X_TO_Y)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.x
						,m_objResize.transform.localScale.x * fRatio
						,m_objResize.transform.localScale.z);
			}
			else if( m_ScaleValue == MEDIA_SCALE.SCALE_X_TO_Z)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.x
						,m_objResize.transform.localScale.y
						,m_objResize.transform.localScale.x * fRatio);
			}
			else if( m_ScaleValue == MEDIA_SCALE.SCALE_Y_TO_X)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.y / fRatio
						,m_objResize.transform.localScale.y
						,m_objResize.transform.localScale.z);
			}
			else if( m_ScaleValue == MEDIA_SCALE.SCALE_Y_TO_Z)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.x
						,m_objResize.transform.localScale.y
						,m_objResize.transform.localScale.y / fRatio);
			}
			else if( m_ScaleValue == MEDIA_SCALE.SCALE_Z_TO_X)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.z * fRatio
						,m_objResize.transform.localScale.y
						,m_objResize.transform.localScale.z);
			}
			else if( m_ScaleValue == MEDIA_SCALE.SCALE_Z_TO_Y)
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.x
						,m_objResize.transform.localScale.z * fRatio
						,m_objResize.transform.localScale.z);
			}
			else 
			{
				m_objResize.transform.localScale 
				= new Vector3(m_objResize.transform.localScale.x,m_objResize.transform.localScale.y,m_objResize.transform.localScale.z);
			}
					
		}
	}
	
	
	
	//The error code is the following sites related documents.
	//http://developer.android.com/reference/android/media/MediaPlayer.OnErrorListener.html 
	void OnError ( MEDIAPLAYER_ERROR iCode , MEDIAPLAYER_ERROR iCodeExtra)
	{
		string strError = "";
		
		switch (iCode)
        {
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK:
            	strError = "MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK";
                break;
			case MEDIAPLAYER_ERROR.MEDIA_ERROR_SERVER_DIED:
            	strError = "MEDIA_ERROR_SERVER_DIED";
                break;
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_UNKNOWN:
            	strError = "MEDIA_ERROR_UNKNOWN";
                break;
            default:
            	strError = "Unknown error " + iCode;
				break;
        }
		
		strError += " ";
		
		switch (iCodeExtra)
        {
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_IO:
            	strError += "MEDIA_ERROR_IO";
                break;
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_MALFORMED:
            	strError += "MEDIA_ERROR_MALFORMED";
                break;
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_TIMED_OUT:
            	strError += "MEDIA_ERROR_TIMED_OUT";
                break;
            case MEDIAPLAYER_ERROR.MEDIA_ERROR_UNSUPPORTED:
            	strError += "MEDIA_ERROR_UNSUPPORTED";
                break;
            default:
            	strError = "Unknown error " + iCode;
				break;
        }
        Debug.LogError(strError);

        if (OnVideoError != null)
        {
            OnVideoError(iCode, iCodeExtra);
        }
	}
	
	
	void OnDestroy()
	{
#if UNITY_ANDROID

		//Call_Reset();
	
#endif
		Call_UnLoad();
		
		if(m_VideoTextureDummy != null)
		{
			Destroy(m_VideoTextureDummy);
			m_VideoTextureDummy = null;
		}
		
		if(m_VideoTexture != null)
			Destroy(m_VideoTexture);
		
		Call_Destroy();


	}

    bool m_bPause = false;
    void OnApplicationPause(bool bPause)
	{
		if(bPause == true)
		{
			if( m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
			{
				m_bPause = true;
			}
#if (UNITY_IPHONE  || UNITY_TVOS)
			m_iPauseFrame = m_iCurrentSeekPosition;
			Stop();


#else
            Pause();
#endif


		}
		else
		{
#if ( UNITY_IPHONE  || UNITY_TVOS )
			m_bStop = false;
			Call_Play(m_iPauseFrame);
#else
			Replay ();
#endif

		}
		
	}

    void makeRequest(List<string> urlList)
    {
        for(int i = 0; i < urlList.Count; i++)
        {
            makeRequest(urlList[i]);
        }
    }

    void makeRequest(string url)
    {
        if (url.Contains("$TS"))
        {
            Int64 unixTimestamp = (Int64)(DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1))).TotalMilliseconds;
            url = url.Replace("$TS", unixTimestamp.ToString());
        }
        StartCoroutine(WaitForRequest(url, 0));
    }

    IEnumerator WaitForRequest(string url, int iterationRequest)
    {
        WWW www = new WWW(url);
        yield return www;
        // check for errors
        if (www.error != null)
        {
            if (iterationRequest < 5)
            {
                iterationRequest++;
                float timeNow = Time.realtimeSinceStartup;
                while (Time.realtimeSinceStartup - timeNow < 1.0f)
                {
                    yield return 0;
                }
                StartCoroutine(WaitForRequest(url, iterationRequest));
            }
        }
    }

    public void ShowStripes(bool show)
    {
        if (progressBarRendererInternalVideo != null)
        {
            progressBarRendererExternalVideo.enabled = show;
            progressBarRendererInternalVideo.enabled = show;
        }
    }

    void OnEnable()
    {
        CentralAdManager.VadrPlay += playVideo;
        CentralAdManager.VadrPause += pauseVideo;
        CentralAdManager.VadrClick += clickVideo;
        AdFetch.VadrLoad += loadVideo;
    }


    void OnDisable()
    {
        CentralAdManager.VadrPlay -= playVideo;
        CentralAdManager.VadrPause -= pauseVideo;
        CentralAdManager.VadrClick -= clickVideo;
        AdFetch.VadrLoad -= loadVideo;
        if( GetCurrentState() == MEDIAPLAYER_STATE.PLAYING)
		{
			Pause();
		}
    }

    void playVideo(int instanceID, float volume)
    {
        if (instanceID == m_TargetMaterial.GetInstanceID())
        {
            if(m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
            {
                Play();
            }
            SetVolume(volume);
        }
    }

    void loadVideo(int instanceID, VastParser vast)
    {
        if(m_TargetMaterial != null)
        {
            if (instanceID == m_TargetMaterial.GetInstanceID())
            {
                Load(vast.inLineVideo.media[0].videoPath, vast);
            }
        }
    }

    void pauseVideo(int instanceID)
    {
        if (m_TargetMaterial != null)
        {
            if (instanceID == m_TargetMaterial.GetInstanceID())
            {
                if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
                {
                    Pause();
                }
            }
        }
    }

    void clickVideo(int instanceID)
    {
        if (instanceID == m_TargetMaterial.GetInstanceID())
        {
            Click();
        }
    }

    IEnumerator CheckProgress()
    {
        bool flagFirst = false;
        bool flagMid = false;
        bool flagThird = false;
        bool flagProgress = false;
        int videoDuration = -1;
        int progressDuration = -1;
        if (vast.inLineVideo.duration.Length > 0)
        {
            string[] parts = vast.inLineVideo.duration.Split(':');
            if (parts.Length >= 3)
            {
                videoDuration = Int32.Parse(parts[0]) * 3600 + Int32.Parse(parts[1]) * 60 + Int32.Parse(parts[2]);
            }
            else if (parts.Length == 2)
            {
                videoDuration = Int32.Parse(parts[1]) * 60 + Int32.Parse(parts[2]);
            }
            else if (parts.Length == 0)
            {
                videoDuration = Int32.Parse(vast.inLineVideo.duration);
            }
        }
        if (vast.inLineVideo.progress.Length > 0)
        {
            if (vast.inLineVideo.progress.Contains("%"))
            {
                string[] parts = vast.inLineVideo.duration.Split('%');
                int percent = Int32.Parse(parts[0]);
                progressDuration = (videoDuration * percent) / 100;
            }
            else if (vast.inLineVideo.progress.Contains(":"))
            {
                string[] parts = vast.inLineVideo.progress.Split(':');
                if (parts.Length >= 3)
                {
                    progressDuration = Int32.Parse(parts[0]) * 3600 + Int32.Parse(parts[1]) * 60 + Int32.Parse(parts[2]);
                }
                else if (parts.Length == 2)
                {
                    progressDuration = Int32.Parse(parts[1]) * 60 + Int32.Parse(parts[2]);
                }
            }
        }
        if (videoDuration >= 0)
        {
            while (m_CurrentState != MEDIAPLAYER_STATE.END || m_CurrentState != MEDIAPLAYER_STATE.ERROR)
            {
                float duration = GetCurrentPosition();
                if (progressBarInternalVideo != null)
                {
                    ShowStripes(true);
                    float scale = progressBarVideo.transform.localScale.x;
                    progressBarInternalVideo.transform.localScale = new Vector3(Mathf.Min((duration / videoDuration) * 1.5f,scale), progressBarInternalVideo.transform.localScale.y, progressBarInternalVideo.transform.localScale.z);
                    progressBarInternalVideo.transform.localPosition = new Vector3(0.75f - Mathf.Min(((duration / videoDuration) * 0.75f),scale/2), progressBarInternalVideo.transform.localPosition.y, progressBarInternalVideo.transform.localPosition.z);
                }
                //UpdateVideoProgress(videoDuration,duration);
                if (duration >= videoDuration / 4 && !flagFirst)
                {
                    flagFirst = true;
                    makeRequest(vast.inLineVideo.firstQuartileUrl);
                }
                if (duration >= videoDuration / 2 && !flagMid)
                {
                    flagMid = true;
                    makeRequest(vast.inLineVideo.midpointUrl);
                }
                if (duration >= 3 * videoDuration / 4 && !flagThird)
                {
                    flagThird = true;
                    makeRequest(vast.inLineVideo.thirdQuartileUrl);
                }
                if(progressDuration >= 0 && duration >= progressDuration && !flagProgress)
                {
                    flagProgress = true;
                    makeRequest(vast.inLineVideo.progressUrl);
                }
                if (progressDuration >= 0 && duration >= videoDuration && !flagProgress)
                {
                    flagProgress = true;
                    makeRequest(vast.inLineVideo.progressUrl);
                }
                float timeNow = Time.realtimeSinceStartup;
                while (Time.realtimeSinceStartup - timeNow < 0.2f)
                {
                    yield return 0;
                }
            }
        }
    }

    public MEDIAPLAYER_STATE GetCurrentState()
	{
		return m_CurrentState;
	}
	
	public Texture2D GetVideoTexture()
	{
		return m_VideoTexture;
	}

    public void Replay()
    {
        if (m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
        {
            Call_RePlay();
            if (vast.inLineVideo.resumeUrl.Count > 0)
            {
                makeRequest(vast.inLineVideo.resumeUrl);
            }
        }
    }
	
	public void Play()
	{
		if(m_bStop == true)
		{
			Call_Play(0);
			m_bStop = false;
		}
		
		if(m_CurrentState == MEDIAPLAYER_STATE.PAUSED  )
		{
            //Call_RePlay();
            Replay();
		}
		else if( m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.STOPPED || m_CurrentState == MEDIAPLAYER_STATE.END)
		{
			Call_Play(0);
		}
	}
	
	public void Stop()
	{
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
            Pause();
			//Call_Pause();
		
	
		m_bStop = true;
		m_CurrentState = MEDIAPLAYER_STATE.STOPPED;
		m_iCurrentSeekPosition = 0;
	}
	
	public void Pause()
	{
		if(m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
        {
            Call_Pause();
            if (vast.inLineVideo.pauseUrl.Count > 0)
            {
                makeRequest(vast.inLineVideo.pauseUrl);
            }
        }
		
		m_CurrentState = MEDIAPLAYER_STATE.PAUSED;
	}

    public void LoadVastXml(string vastXml)
    {
        string mediaFile = Call_LoadVastXml(vastXml);
        if (Application.platform == RuntimePlatform.Android)
        {
            AndroidJavaClass jc = new AndroidJavaClass("com.vadrnet.unitysdk.MainActivity");
            jc.CallStatic("load2DVideo", mediaFile);
        }
        Load(mediaFile);
    }

    public void Click()
    {
        string url = "";
        if(m_CurrentState != MEDIAPLAYER_STATE.END)
        {
            url = vast.inLineVideo.landingPage;
            if (vast.inLineVideo.clickTrackingUrl.Count > 0)
            {
                makeRequest(vast.inLineVideo.clickTrackingUrl);
            }
        }
        else
        {
            if (companionImg != null)
            {
                url = vast.inLineVideo.companion[0].landingPage;
                makeRequest(vast.inLineVideo.companion[0].clickTrackingUrl);
            }
            else
            {
                url = vast.inLineVideo.landingPage;
                if (vast.inLineVideo.clickTrackingUrl.Count > 0)
                {
                    makeRequest(vast.inLineVideo.clickTrackingUrl);
                }
            }
        }
        isRedirected = true;
        Call_Click(url);
    }

    public void Skip()
    {
        if (vast.inLineVideo.skipUrl.Count > 0)
        {
            makeRequest(vast.inLineVideo.skipUrl);
        }
        //Call_Skip();
    }

    public void Load(string strFileName)
	{
		m_bCheckFBO = false;
        m_bFirst = true;
        m_strFileName = strFileName;

#if UNITY_ANDROID



        //Call_Reset();



#endif


#if UNITY_IPHONE
		if (strFileName.StartsWith("http",StringComparison.OrdinalIgnoreCase))
		{
			StartCoroutine(DownloadStreamingVideoAndLoad(m_strFileName));
		}
		else
		{
			Call_Load(m_strFileName,0);
		}
			
#endif

#if UNITY_ANDROID

        if (m_bSupportRockchip)
		{
			Call_SetRockchip(m_bSupportRockchip);
			
			if(strFileName.Contains("://"))
			{
				Call_Load(strFileName,0);
			}
			else
			{
				StartCoroutine( CopyStreamingAssetVideoAndLoad(strFileName));
			}
		
		}
		else
		{
			Call_Load(strFileName,0);
		}
		
	 		
#endif
		
		
		m_CurrentState = MEDIAPLAYER_STATE.NOT_READY;
	}


    IEnumerator downloadImage(string imageUri)
    {
        WWW www = new WWW(imageUri);
        yield return www;
        if (string.IsNullOrEmpty(www.error))
        {
            companionImg = www.bytes;
            //changeTexture(www.bytes);
        }
        else
        {
            //adRendered = false;
            print("error:" + www.error);
        }
    }

    void changeTexture(byte[] image)
    {
        Texture2D tex = new Texture2D(2, 2);
        tex.LoadImage(image);
        GetComponent<Renderer>().material.mainTexture = tex;
    }
	void changeTextureAfterInverting(byte[] image){
		if (Application.platform == RuntimePlatform.IPhonePlayer) {
			Texture2D tex = new Texture2D (2, 2);
			tex.LoadImage (image);
			int width = tex.width;
			int height = tex.height;
			Texture2D invertedTex = new Texture2D (width, height);
			for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					invertedTex.SetPixel (i, height - j - 1, tex.GetPixel (i, j));
				}
			}
			invertedTex.Apply ();
			GetComponent<Renderer> ().material.mainTexture = invertedTex;
		} else {
			changeTexture (image);
		}
	}

    public void Load(string strFileName, VastParser vastParsed)
    {
        vast = vastParsed;
        m_bCheckFBO = false;
        m_bFirst = true;
        m_strFileName = strFileName;
        companionImg = null;
        if(vast.inLineVideo.companion.Count > 0)
        {
            StartCoroutine(downloadImage(vast.inLineVideo.companion[0].staticResource));
        }
#if UNITY_ANDROID

        //Call_Reset();

#endif

#if UNITY_IPHONE
		if (strFileName.StartsWith("http",StringComparison.OrdinalIgnoreCase))
		{
			StartCoroutine(DownloadStreamingVideoAndLoad(m_strFileName));
		}
		else
		{
			Call_Load(m_strFileName,0);
		}
			
#endif

#if UNITY_ANDROID

        if (m_bSupportRockchip)
        {
            Call_SetRockchip(m_bSupportRockchip);

            if (strFileName.Contains("://"))
            {
                Call_Load(strFileName, 0);
            }
            else
            {
                StartCoroutine(CopyStreamingAssetVideoAndLoad(strFileName));
            }

        }
        else
        {
            Call_Load(strFileName, 0);
        }
#endif


        m_CurrentState = MEDIAPLAYER_STATE.NOT_READY;
    }

    string detectRedirect(string url)
    {
        if (url.Contains("market://"))
        {
            return url;
        }
        try
        {
            ServicePointManager.ServerCertificateValidationCallback = delegate { return true; };
            HttpWebRequest req = (HttpWebRequest)WebRequest.Create(url);
            req.AllowAutoRedirect = false;
            req.Timeout = 1200;
            HttpWebResponse myResp = (HttpWebResponse)req.GetResponse();
            while (myResp.StatusCode == HttpStatusCode.Redirect)
            {
                if (url.Contains("market://"))
                {
                    return url;
                }
                Uri uri = new Uri(url);
                string host = uri.Scheme + Uri.SchemeDelimiter + uri.Host;
                ServicePointManager.ServerCertificateValidationCallback = delegate { return true; };
                url = myResp.GetResponseHeader("Location");
                if (url[0] == '/')
                {
                    url = host + url;
                }
                req = (HttpWebRequest)WebRequest.Create(url);
                req.AllowAutoRedirect = false;
                req.Timeout = 1200;
                myResp = (HttpWebResponse)req.GetResponse();
            }
            if ((int)myResp.StatusCode > 400)
            {
                Debug.Log("Wrong: " + myResp.StatusCode);
                return "";
            }
            return url;
        }
        catch (WebException e)
        {
            Debug.Log(e.Message);
            if (e.Status == WebExceptionStatus.Timeout)
            {
                return url;
            }
            return "";
        }
    }


    public void SetVolume(float fVolume)
	{
        if (fVolume == 0 && previousVolume != 0)
        {
            if(vast.inLineVideo.muteUrl.Count > 0)
            {
                makeRequest(vast.inLineVideo.muteUrl);
            }
        }
        else if(fVolume != 0 && previousVolume == 0)
        {
            if (vast.inLineVideo.unmuteUrl.Count > 0)
            {
                makeRequest(vast.inLineVideo.unmuteUrl);
            }
        }
		Call_SetVolume(fVolume);
        previousVolume = fVolume;
	}
	
	//return milisecond  
	public int GetSeekPosition()
	{
        if( m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED  || m_CurrentState == MEDIAPLAYER_STATE.END )
			return m_iCurrentSeekPosition;
		else 
			return 0;
	}
	
	public void SeekTo(int iSeek)
	{
		if( m_CurrentState == MEDIAPLAYER_STATE.PLAYING|| m_CurrentState == MEDIAPLAYER_STATE.READY  || m_CurrentState == MEDIAPLAYER_STATE.PAUSED  || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
			Call_SetSeekPosition(iSeek);
	}
	
 
	//Gets the duration of the file.
	//Returns
	//the duration in milliseconds, if no duration is available (for example, if streaming live content), -1 is returned.
	public int GetDuration()
	{
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
            return Call_GetDuration();
        else
            return 0;
	}


	public float GetSeekBarValue()
	{
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
		{
			if (GetDuration () == 0) {
				return 0;
			}
			return (float)GetSeekPosition() / (float)GetDuration() ;
		}
		else
			return 0;
	}

	public void SetSeekBarValue(float fValue)
	{
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
		{
			if (GetDuration () == 0) {
				return;
			}

			SeekTo( (int)((float)GetDuration() * fValue) );
		}
		else
			return;
	}


    public float GetCurrentPosition()
    {
        return Call_GetCurrentPosition();
    }

    //Get update status in buffering a media stream received through progressive HTTP download. 
    //The received buffering percentage indicates how much of the content has been buffered or played. 
    //For example a buffering update of 80 percent when half the content has already been played indicates that the next 30 percent of the content to play has been buffered.
    //the percentage (0-100) of the content that has been buffered or played thus far 
    public int GetCurrentSeekPercent()
	{
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.READY)
            return Call_GetCurrentSeekPercent();
        else
            return 0;
	}
	
	public int GetVideoWidth()
	{
		return Call_GetVideoWidth();
	}
	
	public int GetVideoHeight()
	{
		return Call_GetVideoHeight();
	}
	
	public void UnLoad()
	{
		m_bCheckFBO = false;
#if UNITY_ANDROID
	
		//Call_Reset();
#endif
		Call_UnLoad();
		
		m_CurrentState = MEDIAPLAYER_STATE.NOT_READY;

	}





#if !UNITY_EDITOR && !UNITY_STANDALONE

#if UNITY_ANDROID

    private AndroidJavaObject javaObj = null;

    private AndroidJavaObject GetJavaObject()
    {
        if (javaObj == null)
        {
            javaObj = new AndroidJavaObject("com.EasyMovieTexture.EasyMovieTexture");
        }
        //if (javaObj == null)
        //{
        //    javaObj = javaObjAds.Call<AndroidJavaObject>("getMovieObj");
        //}
        return javaObj;
    }

   

  
	private void Call_Destroy()
	{
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{
#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 5 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(5 + m_iAndroidMgrID * 10 + 7000);
#endif

		}
		else
		{
			GetJavaObject().Call("Destroy");
		}
#else
		GetJavaObject().Call("Destroy");
#endif
	}
    private void Call_Skip()
	{
		GetJavaObject().Call("skip");
	}
	
	
	private void Call_UnLoad()
	{
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{


#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 4 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(4 + m_iAndroidMgrID * 10 + 7000);
#endif
		}
		else
		{
			GetJavaObject().Call("UnLoad");
		}
#else
		GetJavaObject().Call("UnLoad");
#endif
	}
	
	private bool Call_Load(string strFileName, int iSeek)
	{
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{
			GetJavaObject().Call("NDK_SetFileName", strFileName);

#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 1 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(1+ m_iAndroidMgrID * 10 + 7000);
#endif


			Call_SetNotReady();
			
			return true;
		}
		else
		{
			GetJavaObject().Call("NDK_SetFileName", strFileName);
			if (GetJavaObject().Call<bool>("Load"))
			{
				return true;
			}
			else
			{
				OnError(MEDIAPLAYER_ERROR.MEDIA_ERROR_UNKNOWN, MEDIAPLAYER_ERROR.MEDIA_ERROR_UNKNOWN);
				return false;
			}
		}
#else
		GetJavaObject().Call("NDK_SetFileName", strFileName);
		if (GetJavaObject().Call<bool>("Load"))
		{
			return true;
		}
		else
		{
			OnError(MEDIAPLAYER_ERROR.MEDIA_ERROR_UNKNOWN, MEDIAPLAYER_ERROR.MEDIA_ERROR_UNKNOWN);
			return false;
		}
#endif
	}

    private string Call_LoadVastXml(string xmlString)
	{
		return GetJavaObject().Call<string>("LoadVastXml", xmlString);
	}

    private void Call_Click(string url)
	{
        url = detectRedirect(url);
        if(url.Length > 0){
            if(url.Contains("play.google.com") || url.Contains("market://")){
            if(VadrAppStore != null)
		        VadrAppStore(url);
            }
            else{
                GetJavaObject().Call("Click",url);
            }
        }
	}
	
	private void Call_UpdateVideoTexture()
	{
		if( Call_IsUpdateFrame() == false)
			return;
		
		if(m_VideoTextureDummy != null)
		{
			Destroy(m_VideoTextureDummy);
			m_VideoTextureDummy = null;
		}


		if(m_TargetMaterial)
		{
			if(m_TargetMaterial.GetComponent<MeshRenderer>()!= null)
			{
				if(m_TargetMaterial.GetComponent<MeshRenderer>().material.mainTexture != m_VideoTexture)
				{
					m_TargetMaterial.GetComponent<MeshRenderer>().material.mainTexture = m_VideoTexture;
				}
			}
		}

		
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{
#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 3 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(3+ m_iAndroidMgrID * 10 + 7000);
#endif

		
		}
		else
		{
			GetJavaObject().Call("UpdateVideoTexture");
		}
#else
		GetJavaObject().Call("UpdateVideoTexture");
#endif
		
		if (!m_bIsFirstFrameReady)
		{
			m_bIsFirstFrameReady = true;
			if (OnVideoFirstFrameReady != null)
			{
				OnVideoFirstFrameReady();
				OnVideoFirstFrameReady = null;
			}
		}
	}
	
	private void Call_SetVolume(float fVolume)
	{
		GetJavaObject().Call("SetVolume",fVolume);
	}
	
	private void Call_SetSeekPosition(int iSeek)
	{
		GetJavaObject().Call("SetSeekPosition",iSeek);
	}
	
	private int Call_GetSeekPosition()
	{
		return GetJavaObject().Call<int>("GetSeekPosition");
	}
	
	private void Call_Play(int iSeek)
	{
		GetJavaObject().Call("Play",iSeek);
	}
	
	private void Call_Reset()
	{
		GetJavaObject().Call("Reset");
	}
	
	private void Call_Stop()
	{
		GetJavaObject().Call("Stop");
	}
	
	private void Call_RePlay()
	{
		GetJavaObject().Call("RePlay");
	}
	
	private void Call_Pause()
	{
		GetJavaObject().Call("Pause");
	}
	
	private int Call_InitNDK()
	{
		return GetJavaObject().Call<int>("InitNative",GetJavaObject());
	}

	private int Call_GetVideoWidth()
	{
		return GetJavaObject().Call<int>("GetVideoWidth");
	}
	
	private int Call_GetVideoHeight()
	{
		return GetJavaObject().Call<int>("GetVideoHeight");
	}
		
	private bool Call_IsUpdateFrame()
	{
		return GetJavaObject().Call<bool>("IsUpdateFrame");
	}
	
	private void Call_SetUnityTexture(int iTextureID)
	{
		GetJavaObject().Call("SetUnityTexture",iTextureID);
	}
	
	private void Call_SetWindowSize()
	{
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{


#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 2 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(2+ m_iAndroidMgrID * 10 + 7000);
#endif
		}
		else
		{
			GetJavaObject().Call("SetWindowSize");
		}
#else
		GetJavaObject().Call("SetWindowSize");
#endif
	}
	
	private void Call_SetLooping(bool bLoop)
	{
		GetJavaObject().Call("SetLooping",bLoop);
	}
	
	private void Call_SetRockchip(bool bValue)
	{
		GetJavaObject().Call("SetRockchip",bValue);
	}
	
	
	private int Call_GetDuration()
	{
		return GetJavaObject().Call<int>("GetDuration");
	}
	
    private float Call_GetCurrentPosition()
	{
		return GetJavaObject().Call<float>("GetCurrentPosition");
	}

	private int Call_GetCurrentSeekPercent()
	{
		return GetJavaObject().Call<int>("GetCurrentSeekPercent");
	}
	
	private int Call_GetError()
	{
		return GetJavaObject().Call<int>("GetError");
	}

	private void Call_SetSplitOBB(bool bValue, string strOBBName)
	{
		GetJavaObject().Call("SetSplitOBB",bValue,strOBBName);
	}
	
	private int Call_GetErrorExtra()
	{
		return GetJavaObject().Call<int>("GetErrorExtra");
	}
	
	
	private void Call_SetUnityActivity()
    {
        AndroidJavaClass jc = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
		AndroidJavaObject jo = jc.GetStatic<AndroidJavaObject>("currentActivity");
		GetJavaObject().Call("SetUnityActivity", jo);
		
#if UNITY_5
		if( SystemInfo.graphicsMultiThreaded == true)
		{
#if UNITY_5_2
			GL.IssuePluginEvent(EasyMovieTextureRender(), 0 + m_iAndroidMgrID * 10 + 7000);
#else
			GL.IssuePluginEvent(0+ m_iAndroidMgrID * 10 + 7000);
#endif
		}
		else
		{
			Call_InitJniManager();
		}
#else
		Call_InitJniManager();
#endif
    }
	
	private void Call_SetNotReady()
	{
		GetJavaObject().Call("SetNotReady");
	}
	
	private void Call_InitJniManager()
	{
		GetJavaObject().Call("InitJniManager");
	}

	private MEDIAPLAYER_STATE Call_GetStatus()
	{
		return (MEDIAPLAYER_STATE)GetJavaObject().Call<int>("GetStatus");
	}
    
    
	
#elif UNITY_IPHONE || UNITY_TVOS
	[DllImport("__Internal")]
	private static extern int VadrVideoPlayerPluginCreateInstance();
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginDestroyInstance(int iID);
	[DllImport("__Internal")]
	private static extern bool VadrVideoPlayerPluginCanOutputToTexture(string videoURL);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginSetLoop(int iID, bool bLoop);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginSetVolume(int iID, float fVolume);
	[DllImport("__Internal")]
	private static extern bool VadrVideoPlayerPluginPlayerReady(int iID);
	[DllImport("__Internal")]
	private static extern float VadrVideoPlayerPluginDurationSeconds(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginExtents(int iID,ref int width, ref int height);
	[DllImport("__Internal")]
	private static extern IntPtr VadrVideoPlayerPluginCurFrameTexture(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginLoadVideo(int iID,string videoURL);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginPlayVideo(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginPauseVideo(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginResumeVideo(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginRewindVideo(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginSeekToVideo(int iID,float time);
	[DllImport("__Internal")]
	private static extern float VadrVideoPlayerPluginCurTimeSeconds(int iID);
	[DllImport("__Internal")]
	private static extern bool VadrVideoPlayerPluginIsPlaying(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginStopVideo(int iID);
	[DllImport("__Internal")]
	private static extern bool VadrVideoPlayerPluginFinish(int iID);
	[DllImport("__Internal")]
	private static extern bool VadrVideoPlayerPluginError(int iID);
	[DllImport("__Internal")]
	private static extern void VadrVideoPlayerPluginSetTexture(int iID,int iTextureID);
	
	
	
	
	
	int m_iID = -1;
	/// <summary>
	/// ???????????????true???????    /// </summary>
	public bool ready
	{
		get
		{
		return VadrVideoPlayerPluginPlayerReady(m_iID);
		}
	}
	
	/// <summary>
	/// 
	/// </summary>
	public float duration
	{
		get
		{
		return VadrVideoPlayerPluginDurationSeconds(m_iID);
		}
	}
	
	
	public float currentTime
	{
		get
		{
		return VadrVideoPlayerPluginCurTimeSeconds(m_iID);
		}
	}
	
	
	public bool isPlaying
	{
		get
		{
		return VadrVideoPlayerPluginIsPlaying(m_iID);
		}
	}
	public Vector2 videoSize
	{
		get
		{
			int width = 0, height = 0;
		VadrVideoPlayerPluginExtents(m_iID,ref width, ref height);
			return new Vector2(width, height);
		}
	}
	
	private Texture2D _videoTexture;
	
	public Texture2D videoTexture
	{
		get
		{
		IntPtr nativeTex = ready ? VadrVideoPlayerPluginCurFrameTexture(m_iID) : (IntPtr)0;
			
			
			if (nativeTex != (IntPtr)0)
			{
				if (_videoTexture == null)
				{
					_videoTexture = Texture2D.CreateExternalTexture((int)videoSize.x, (int)videoSize.y, TextureFormat.RGBA32,
					                                                false, false, (IntPtr)nativeTex);
					_videoTexture.filterMode = FilterMode.Bilinear;
					_videoTexture.wrapMode = TextureWrapMode.Clamp;
				}
				
				_videoTexture.UpdateExternalTexture((IntPtr)nativeTex);
			}
			/* else
            {
				if(_videoTexture != null)
				{
					Destroy(_videoTexture);
				}
				
                _videoTexture = null;
            }*/
			
			return _videoTexture;
		}
	}
	
	private void Call_Destroy()
	{
		
	VadrVideoPlayerPluginDestroyInstance(m_iID);
		
		if(_videoTexture != null)
			Destroy(_videoTexture);
		
		_videoTexture = null;
		m_iID = -1;
	}
	
	private void Call_UnLoad()
	{
		
	VadrVideoPlayerPluginStopVideo(m_iID);
	VadrVideoPlayerPluginDestroyInstance(m_iID);
	}
	
	private bool Call_Load(string strFileName, int iSeek)
	{
		if( m_iID == -1)
		{
			Call_SetUnityActivity();
		}

		if(_videoTexture != null)
		{
			Destroy(_videoTexture);
		}
		
		_videoTexture = null;
		
	if (VadrVideoPlayerPluginCanOutputToTexture(strFileName))
		{
		VadrVideoPlayerPluginLoadVideo(m_iID,strFileName);	
		}
		
		
		
		
		return true;
	}
	
	private void Call_UpdateVideoTexture()
	{
		
		if(m_VideoTextureDummy != null)
		{
			Destroy(m_VideoTextureDummy);
			m_VideoTextureDummy = null;
		}
		
		if( m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
		{

			if( videoTexture == null)
				return;

			if(m_TargetMaterial)
			{
				if(m_TargetMaterial.GetComponent<MeshRenderer>() != null)
					m_TargetMaterial.GetComponent<MeshRenderer>().material.mainTexture = _videoTexture;
			}

			
			m_VideoTexture = _videoTexture;
		}
		
		if (!m_bIsFirstFrameReady)
		{
			m_bIsFirstFrameReady = true;
			if (OnVideoFirstFrameReady != null)
			{
				OnVideoFirstFrameReady();
				OnVideoFirstFrameReady = null;
			}
		}
		
	
		
		
		
		
	}
	
	private void Call_SetVolume(float fVolume)
	{
	VadrVideoPlayerPluginSetVolume(m_iID, fVolume);
	}
	
	private void Call_SetSeekPosition(int iSeek)
	{
		float fSeek = (float)iSeek / 1000.0f;
	VadrVideoPlayerPluginSeekToVideo(m_iID,fSeek);
	}
	
	private int Call_GetSeekPosition()
	{
		
	float fSeek = VadrVideoPlayerPluginCurTimeSeconds(m_iID);
		return (int)(fSeek * 1000.0f);
	}
	
	private void Call_Play(int iSeek)
	{
		float fSeek = (float)iSeek / 1000.0f;
		
		if( isPlaying == true )
		{
		VadrVideoPlayerPluginSeekToVideo(m_iID,fSeek);
		}
		else
		{
			if( m_CurrentState != MEDIAPLAYER_STATE.READY)
			VadrVideoPlayerPluginSeekToVideo(m_iID,fSeek);
			
		if (!iPhoneTextureInverted) {
			if(m_TargetMaterial.GetComponent<MeshFilter>() != null)
			{
				iPhoneTextureInverted = true;
				Debug.Log("Video debug: Inverting texture");
				Vector2 [] vec2UVs= m_TargetMaterial.GetComponent<MeshFilter>().mesh.uv;

				for(int i = 0; i < vec2UVs.Length; i++)
				{
					vec2UVs[i] = new Vector2(vec2UVs[i].x, 1.0f -vec2UVs[i].y);
				}

				m_TargetMaterial.GetComponent<MeshFilter>().mesh.uv = vec2UVs;
			}

		}
		VadrVideoPlayerPluginPlayVideo(m_iID);
		}
		
		
		if( m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
			m_CurrentState = MEDIAPLAYER_STATE.PLAYING;
		
		
	}
	
	private void Call_Reset()
	{
		
	}
	
	private void Call_Stop()
	{
	VadrVideoPlayerPluginStopVideo(m_iID);
	}
	
	private void Call_RePlay()
	{
	VadrVideoPlayerPluginResumeVideo(m_iID);
		m_CurrentState = MEDIAPLAYER_STATE.PLAYING;
	}
	
	private void Call_Pause()
	{
		m_CurrentState = MEDIAPLAYER_STATE.PAUSED;
	VadrVideoPlayerPluginPauseVideo(m_iID);
		
	}
	
	private int Call_GetVideoWidth()
	{
		return (int)videoSize.x;
	}
	
	private int Call_GetVideoHeight()
	{
		return (int)videoSize.y;
	}
	
	private void Call_SetUnityTexture(int iTextureID)
	{
	VadrVideoPlayerPluginSetTexture(m_iID,iTextureID);
	}
	
	private void Call_SetWindowSize()
	{
		
	}
	
	private void Call_SetLooping(bool bLoop)
	{
	VadrVideoPlayerPluginSetLoop(m_iID, bLoop);
	}
	
	
	public void Call_SetUnityActivity()
	{
		m_iID = VadrVideoPlayerPluginCreateInstance();
	}
	
	private int Call_GetError()
	{
		return 0;
	}
	
	private int Call_GetErrorExtra()
	{
		return 0;
	}
	
	private int Call_GetDuration()
	{
		return (int)(duration * 1000);
	}
	
	private int Call_GetCurrentSeekPercent()
	{
		return -1;
	}
	
	private MEDIAPLAYER_STATE Call_GetStatus()
	{
	if( VadrVideoPlayerPluginFinish(m_iID) )
		{
			return MEDIAPLAYER_STATE.END;
		}

	if( VadrVideoPlayerPluginError(m_iID) )
		{
			return MEDIAPLAYER_STATE.ERROR;
		}
		
		if( m_CurrentState == MEDIAPLAYER_STATE.PAUSED || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
		{
			return m_CurrentState;
		}
		else if(isPlaying)
		{
			return MEDIAPLAYER_STATE.PLAYING;
		}
		else if(ready)
		{
			return MEDIAPLAYER_STATE.READY;
		}
		
		
		return m_CurrentState;
	}
    private void Call_Skip()
    {

    }
    private string Call_LoadVastXml(string xmlString)
    {
        return "";
    }

    private void Call_Click(string url)
    {
		if (VadrVideoiOSWebview != null){
			VadrVideoiOSWebview(url);
		}
    }

    private float Call_GetCurrentPosition()
    {
	return VadrVideoPlayerPluginCurTimeSeconds(m_iID);
    }

#endif
#else
	private void Call_Destroy()
	{
		
	}

	private string Call_LoadVastXml(string xmlString)
    {
        return "";
    }

    private void Call_Click(string url)
    {        
    }
	
	private void Call_UnLoad()
	{
		
	}
	
	private bool Call_Load(string strFileName, int iSeek)
	{
		
		
		return true;
	}
	
	private void Call_UpdateVideoTexture()
	{
		
	}
	
	private void Call_SetVolume(float fVolume)
	{
		
	}
	
	private void Call_SetSeekPosition(int iSeek)
	{
		
	}
	
	private int Call_GetSeekPosition()
	{
		return 0;
	}
	
	private void Call_Play(int iSeek)
	{
		
	}
	
	private void Call_Reset()
	{
		
	}
	
	private void Call_Stop()
	{
		
	}
	
	private void Call_RePlay()
	{
		
	}
	
	private void Call_Pause()
	{
		
	}
	
	private int Call_GetVideoWidth()
	{
		return 0;
	}
	
	private int Call_GetVideoHeight()
	{
		return 0;
	}
	
	private void Call_SetUnityTexture(int iTextureID)
	{
		
	}
	
	private void Call_SetWindowSize()
	{
		
	}
	
	private void Call_SetLooping(bool bLoop)
	{
		
	}
	
	private void Call_SetRockchip(bool bValue)
	{
		
	}
	
	public void Call_SetUnityActivity()
	{
		
	}
	
	private int Call_GetError()
	{
		return 0;
	}
	
	private int Call_GetErrorExtra()
	{
		return 0;
	}
	
	private int Call_GetDuration()
	{
		return -1;
	}
	
	private int Call_GetCurrentSeekPercent()
	{
		return -1;
	}
	
	private void Call_SetSplitOBB(bool bValue, string strOBBName)
	{
	}
	
	private MEDIAPLAYER_STATE Call_GetStatus()
	{
		return (MEDIAPLAYER_STATE)0;
	}

    private void Call_Skip()
    {

    }
    private float Call_GetCurrentPosition()
    {
        return 0.0f;
    }

#endif



    IEnumerator DownloadStreamingVideoAndLoad(string strURL)
	{
		strURL = strURL.Trim();
		
		WWW www = new WWW(strURL);
		
		yield return www;
		
		if(string.IsNullOrEmpty(www.error))
		{

			if( System.IO.Directory.Exists( Application.persistentDataPath + "/Data") == false)
				System.IO.Directory.CreateDirectory( Application.persistentDataPath + "/Data");

			string write_path = Application.persistentDataPath + "/Data" + strURL.Substring(strURL.LastIndexOf("/"));
			
		/*	if(System.IO.File.Exists(write_path) == true)
			{
				Debug.Log("Delete : " + write_path);
				System.IO.File.Delete(write_path);
			}
		*/
			System.IO.File.WriteAllBytes(write_path, www.bytes);
			
			Call_Load("file://"+write_path,0);
		}
		else
		{
			Debug.Log(www.error);
			
		}
		
		www.Dispose();
		www = null;
		Resources.UnloadUnusedAssets();
	}
	
	IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
	{
		strURL = strURL.Trim();
		
		string write_path = Application.persistentDataPath + "/" + strURL;
		
		if(System.IO.File.Exists(write_path) == false)
		{
		
			WWW www = new WWW( Application.streamingAssetsPath +"/"+ strURL);
			
			yield return www;
			
			if( string.IsNullOrEmpty(www.error))
			{
				
				
				
				Debug.Log (write_path);
				System.IO.File.WriteAllBytes(write_path, www.bytes );
				
				Call_Load("file://"+write_path,0);
				
				
			}
			else
			{
				Debug.Log(www.error);
				
			}
			
			www.Dispose();
			www = null;
		}
		else
		{
			Call_Load("file://"+write_path,0);
		}
		
	}
}
